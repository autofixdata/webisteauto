    ...
    <>
      ...
      <header className="sticky top-0 z-50 bg-white border-b border-gray-200 shadow-sm h-16">
        <div className="w-full px-4 lg:px-8 h-full flex items-center justify-between">
          ...
          <form onSubmit={submitSearch} className="hidden md:flex items-center relative max-w-[200px] lg:max-w-[240px] flex-1 mx-2">
            <Search className="w-4 h-4 text-gray-400 absolute left-3 pointer-events-none" />
            <input
              type="search"
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              placeholder="Search codes, makes…"
              className="w-full pl-9 pr-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-afd-yellow/50 focus:border-afd-yellow"
          ...
          <div className="hidden lg:flex items-center gap-4">
            ...
            <div className="relative py-2"
              ...
              <button className="flex items-center gap-1.5 text-xs font-semibold text-afd-slate hover:text-afd-navy transition-colors uppercase tracking-wider">
                <Image src={`https://flagcdn.com/w20/${currentLang.flagCode}.png`} alt="" width={20} height={15} sizes="20px" className="w-5 h-[15px] shadow-sm rounded-[1px]" style={{ width: 'auto', height: '15px' }} />
                {currentLang.code}
              ...
              {isLangDropdownOpen && (
                <div className="absolute top-full right-0 w-36 bg-white border border-gray-100 shadow-xl rounded-xl py-2 mt-0 z-50">
                  {LANGUAGES.map((l) => (
                    ...
                    >
                      <Image src={`https://flagcdn.com/w20/${l.flagCode}.png`} alt="" width={20} height={15} sizes="20px" className="w-5 h-[15px] shadow-sm rounded-[1px]" style={{ width: 'auto', height: '15px' }} />
                      {l.name}
                    </Link>
                  ))}
                </div>
            ...
            <div className="flex-1 overflow-y-auto py-6 px-6 flex flex-col gap-6">
              <form onSubmit={submitSearch} className="relative">
                ...
                <input
                  ...
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  placeholder="Search codes, makes, blog…"
                  className="w-full pl-9 pr-3 py-3 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-afd-yellow/50"
                  aria-label="Search site"
                />
              </form>
            ...
            <div className="px-6 py-4 border-t border-gray-200">
              ...
              <div className="flex flex-wrap gap-2">
                {LANGUAGES.map((l) => (
                  ...
                  >
                    <Image src={`https://flagcdn.com/w20/${l.flagCode}.png`} alt="" width={20} height={15} sizes="20px" className="w-4 h-[12px] shadow-sm" style={{ width: 'auto', height: '12px' }} />
